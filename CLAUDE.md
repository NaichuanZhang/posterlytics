...
- **InsForge client** (`src/lib/insforge.ts`): one shared `insforge` client; `FUNCTIONS_HOST` comes from the pure `resolveFunctionsHost` (`src/lib/functionsHost.ts`) — `VITE_INSFORGE_FUNCTIONS_HOST` if set, else derived from `VITE_INSFORGE_URL` as `<appkey>.functions.insforge.app`. That's where the public `view` function lives, a different host from the API base. The override exists because the provider's function domain is **not stable** (Deno Deploy Classic was sunset 2026-07-20; its replacement serves on `<appkey>.function2.insforge.app`). `buildViewUrl` bakes this host into every minted QR, so changing it does **not** repair already-printed posters — see `docs/decisions/2026-07-31-configurable-functions-host.md`.
...
- **Amazon sources**: `src/lib/amazonSource.ts` recognizes only the documented exact Amazon/official-short-link hosts. The wizard and editor proactively direct these sellers to provide listing copy and product/brand images; Amazon refresh preflight does not promise runtime website assets.
- **Posters**: every poster is the AI-painted `AiPoster`, sized by the dependency-free registry in `src/lib/posterSize.ts`. The default remains 980x1470 artwork on a 1240x1754 A4 sheet. Descriptor mode `scaled` composites a real scannable `QrCode` in a branded bottom band; mode `none` renders full-bleed artwork with no footer.
- **Poster format persistence**: `campaigns.poster_format` is the target for the next generation, while immutable `poster_generations.poster_format` records each version's actual format. Historical render/export must use the generation snapshot.
- **Export**: `PosterExportButton` renders offscreen at the descriptor's native sheet size and captures with its registered pixel ratio. It converts the cross-origin hero to a data URL and awaits image/font readiness to avoid a tainted or blank canvas.

### Edge functions (`functions/`)
Each `.ts` (except `_shared.ts`, `build.mjs`) is one deployable function. **Subhosting uploads a single file and cannot resolve sibling imports**, so `build.mjs` bundles each entry, `_shared.ts`, and pure shared modules such as the poster-size registry into `functions/dist/<slug>.ts`. **Always edit `functions/<slug>.ts` and rebuild — never edit `functions/dist/`.**

- `analyze` — scrapes product copy/assets, captures the creator's light/dark page variant, sends the style board as primary multimodal evidence, and persists richer optional `style_profile`/`design_tokens` JSON. Raw HTML color mining and the scraped-asset analysis fallback both run only when no captured evidence exists — with no style board, the already-re-hosted logo/product images are attached as explicitly *secondary* evidence so the stage is not describing visual treatment from text alone (`docs/decisions/2026-08-16-analyze-source-asset-fallback.md`). Note the scrape itself is unconditional: product copy and images come from raw HTML on every generation, capture or not. Exact classified Amazon sources bypass raw fetch and capture, clear inherited website evidence on the new snapshot without deleting parent objects, and treat seller references as primary. A new non-Amazon board is persisted before its pointer changes; attempt-local uploads are removed only if persistence fails. Historical event rows use the retained bounded Luma parser. Auth-scoped.
...
- `hero` — paints the AI illustration poster at the generation snapshot's registered provider aspect: product references are ordered style board, user images, logo, then product assets, capped at six and labeled by purpose. Products compile `poster_layout` via the pure `compileLayoutPrompt`; events retain their bespoke prompt/reference path. Persists `hero_image_url`/`hero_image_key`. Auth-scoped.
