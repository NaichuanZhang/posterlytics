...
4. Parse destinations with the platform `URL` API for validation and existing-key detection, then append encoded missing parameters to the raw query before its fragment. Existing query bytes remain untouched. Return the original destination byte-for-byte when parsing or decoration throws.
...
4. Structured parsing preserves query semantics for key detection, while raw append avoids normalizing provider-owned values such as Amazon Attribution `%20` bytes into `+`. Redirect availability is more important than attribution, so malformed or unusual unparseable values must fail open.
